...
func _decide(delta: float) -> Vector3:
	...
	var vitesse: float = stats.vitesse * _facteur_vitesse
	var garde: float = stats.distance_garde
	var sur_un_leurre: bool = cible.is_in_group("leurre")

	match _etat:
		Etat.APPROCHE:
			# Une brute n'a pas de distance de garde : elle arrive au contact.
			if distance <= maxf(garde, stats.portee_frappe):
				_etat = Etat.GARDE
...
func _engage(distance: float) -> void:
	if stats.tire:
		if distance <= stats.portee_frappe:
			...
			veut_tirer.emit(global_position, vers, stats.degats)
			_recharge = stats.delai_assaut
			_signale_attaque()
		return
